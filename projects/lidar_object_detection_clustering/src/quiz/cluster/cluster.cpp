...
void proximityPoints(const std::vector<std::vector<float>>& points, 
						int pointIndex,
...
{
	std::vector<int> nearby = tree->search(points[pointIndex], 3.0);
	for(auto it = nearby.begin(); it != nearby.end(); ++it) {
		if(! checked[*it]) {
			...
			proximityPoints(points, *it, checked, tree, distanceTol, cluster);
		}
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for(int i = 0; i < points.size(); ++i) {
		...
		if(! checked[i]) {
			...
			proximityPoints(points, i, checked, tree, distanceTol, cluster);
